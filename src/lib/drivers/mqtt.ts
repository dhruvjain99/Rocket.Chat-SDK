...
export class MQTTDriver extends EventEmitter implements ISocket, IDriver {
  ...
  constructor({
    ...
    super();
    host = "localhost";
    const [, _host = host, , port = 8081] =
      new RegExp("(.*?)(:([0-9]+))?$").exec(host || "localhost:3000") || [];
    this.config = {
      ...config,
      ...moreConfigs,
    ...
    if (/https/.test(host)) {
      ...
      this.socket = new Client(
        (this.config.host || "").replace("http://", "").replace("ws://", ""),
        Number(port),
        path,
        "clientId"
      );
  ...
  connect(options: ISocketOptions): Promise<any> {
    return new Promise((resolve, reject) => {
      this.socket.connect({
        userName: "livechat-guest",
        password:
          "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ2Ijp7InZpc2l0b3JUb2tlbiI6ImFqamVvY2N5dXhweXVlOTg3YzJ0NnMifSwidXNlciI6eyJ2Ijp7InZpc2l0b3JUb2tlbiI6ImFqamVvY2N5dXhweXVlOTg3YzJ0NnMifX0sIm5hbWUiOiJKb2huIERvZSIsImlhdCI6MTUxNjIzOTAyMn0.RTQz72NTgI6qWgQMCNHHaSNS13sDK3cz--ss2_5vAz8",
        onSuccess: resolve,
        onFailure: reject,
        useSSL: /https/.test(this.config.host || ""),
      });
    });
  }
  ...
  unsubscribe(subscription: ISubscription, ...args: any[]): Promise<ISocket> {
    return new Promise((resolve, reject) => {
      this.socket.unsubscribe(subscription.name, [
        ...args,
        (err: any, granted: any) => {
          if (err) {
            return reject(err);
  ...
  onStreamData(name: string, cb: ICallback): Promise<any> {
    return Promise.resolve(
      this.on(name, ({ fields: { args: [message] } }: any) => cb(message))
    ) as any;
  }
